---
title: "20190117_workflow_AH7&AG7combined"
author: "R_Krautz"
date: "17/01/2019"
...
#### previous "Seurat_3.0.2.tar.gz","Seurat_2.3.4.tar.gz"
```{r}
...
## [0.1.] Load conda environment, python & check for 'umap-learn' module
#### on console: 'conda env list', 'conda list -n bioinf_py37'
```{r}
reticulate::conda_list(
    conda = "/binf-isilon/alab/people/rkrautz/bin/anaconda3/condabin/conda"
  )
reticulate::use_condaenv(
    condaenv = "/binf-isilon/alab/people/rkrautz/bin/anaconda3/envs/bioinf_py37/",
    conda = "/binf-isilon/alab/people/rkrautz/bin/anaconda3/condabin/conda",
    required = TRUE
  )
reticulate::py_config()

pks <- reticulate::py_config() %>%
  .[["numpy"]] %>%
  .[["path"]] %>%
  base::dirname()

pk = "umap" #"pyprojroot"
base::list.dirs(path = pks) %>%
  stringr::str_subset(pk)
```

## [0.2.] Save & load environment
#### previous (1.) '20190415.RData' (2.) '20190614.RData' (3.) '20190614_20190415repro.RData'
```{r eval=FALSE}
base::save.image(
  base::paste0(
      here::here("results/20190110_CellRanger_scRNASeq_NanoDam/"),
      "20211129_rerun.RData"
    ),
    compress = TRUE
  )
```
...
#### Note: address strip labels: g1$grobs[[62]]$grobs[[1]]$children[[2]]$children[[1]]$label
#### Note: addess strip colors: g1$grobs[[i]]$grobs[[1]]$children[[1]]$gp$fill
```{r}
...
#### previous "/outs/filtered_gene_bc_matrices/Ensembl_BDGP6_genes.mod"
```{r}
dataset = "AH7"
...
## [2.8.] Save plot
#### previous "20190123_expressionTSNE_AG7AH7combined"
```{r}
for(ext in base::c("pdf", "png")){
    ggplot2::ggsave(
      filename = base::paste0(
...
#### see [2.7.] of 'Data_wrangling' in '20181211_workflow_peakAnno.Rmd'
```{r}
clus <- base::list()
clus[[2]] <- base::c(
    "dpn","ase","cas",
