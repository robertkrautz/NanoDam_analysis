---
title: "20210827_workflow_correlations"
...
## [0.3.] Define selected chromosomes
```{r}
selected_chr <- base::c("2L","2R","3L","3R","4","X","Y")
```

## [0.4.] Read in dm6.chrom.sizes file & subset
...
## [0.5.] str_ignore() helper function
```{r}
...
## [3.0.] Read in datasets across all samples & perform arithmetics()
#### __Deprecated__ due to long runtime
```{r eval=FALSE}
arits <- files %>%
    dplyr::mutate(
      data = purrr::map(
...
#### previous: "20210203_correlation_Notch", "_trimSortBin500Ext150Trim",
```{r}
for(ext in c("pdf", "png")){
    ggplot2::ggsave(
      filename = base::paste0(
          here::here("results/20210907_correlations/"),
...
#### Note: 'Variations' are to be understood in terms of combinatorics
#### Note: To fill the confidence matrix with all 16x16 (=256) options (i.e., incl trim samples), 'repetitions' are allowed
#### Note: "combin <- combn(samples, 2) %>% t"
```{r}
...
#### sam_1 = col1 = "Dam_nanodam_S3_R1_001_trimmed", sam_2 = col2 = "dCTIP_tada_S7_R1_001"
```{r}
all_corr_pcc %>% 
  dplyr::filter(
      (
        sam_1 == "Dam_nanodam_S3_R1_001_trimmed" &
        sam_2 == "dCTIP_tada_S7_R1_001"
      ) |
      (
        sam_1 == "dCTIP_tada_S7_R1_001" &
        sam_2 == "Dam_nanodam_S3_R1_001_trimmed"
      )
    )
```
...
## [5.0.] Identify bedgraph files
#### 8 files: "Cph_NanoDam_1_vs_Dam_NanoDam_1.bgr"; "Cph_TaDa_1_vs_Dam_TaDa_1.bgr" etc.
```{r}
...
fns_ <- purrr::map(
  ...
  base::unlist() %>% 
  stringr::str_subset("gatc.bedgraph$")
  #stringr::str_subset("bedgraph.quant.norm.bedgraph")
```

####  add controls manually
#### 2 files: "Dam_TaDa_2_vs_Dam_TaDa_1.bgr", "Dam_TaDa_2_vs_Dam_TaDa_1.bgr"
```{r}
ctrl_nano <- base::paste0(
    here::here("data/"),
    "20211117_cph_nanodam_DamVsDam/cph_nanodam_DamVsDam_tracks/",
    "Exp_nanodam_S3-vs-Dam_nanodam_ctrl_S10.gatc.bedgraph"
  )
ctrl_tada <- base::paste0(
    here::here("data/"),
    "20211117_cph_tada_DamVsDam/cph_tada_DamVsDam_tracks/",
    "Exp_tada_S9-vs-Dam_tada_ctrl_S8.gatc.bedgraph"
  )
fns <- base::c(fns_,ctrl_nano,ctrl_tada)
```

## [5.1.] Read out info from filenames
```{r}
files <- tibble::tibble(
      path = fns
    ) %>%
...
## [5.2.] Read in datasets all samples separately
#### Evaluate with 'dplyr::mutate(data,rws = purrr::map_int(data,nrow))'
```{r}
data <- files %>%
    dplyr::mutate(
...
## [5.3.] Quantile normalize all samples
```{r}
...
## [6.0.] Build list with all variations of the compared samples
#### Note: 'Variations' are to be understood in terms of combinatorics
#### Note: To fill the confidence matrix with all 16x16 (=256) options, 'repetitions' are allowed
#### Note: "combin <- combn(samples, 2) %>% t"
```{r}
...
## [6.1.] Run correlator() on all variations with repetition
```{r}
all_corr_pcc <- combin %>%
  dplyr::mutate(
    coeff = purrr::pmap_dbl(
...
## [6.2.] Plot all correlations in contingency table
```{r}
cols <- grDevices::colorRampPalette(
    base::rev(RColorBrewer::brewer.pal(11,"RdBu"))
  )(100)
